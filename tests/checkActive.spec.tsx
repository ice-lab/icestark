...
describe('checkActive', () => {
  test('matchPath - path options', () => {
    let match = matchPath('/test/123', { pathData: { value: '/test'} });
    expect(match && match.url).toBe('/test');

    match = matchPath('/test/123', { pathData: { value: '/test/:id' }  });
    expect(match && match.url).toBe('/test/123');
    expect(match && match.params.id).toBe('123');

    // test exact
    match = matchPath('/test/123', { pathData: { value: '/test' }, exact: true });
    ...
    // test sensitive
    match = matchPath('/Test/123', { pathData: { value: '/test',  sensitive: false }});
    expect(match && match.url) .toBe('/Test');

    match = matchPath('/Test/123', { pathData: { value: '/test',  sensitive: true }});
    expect(match).toBeFalsy();

    // test strict
    match = matchPath('/test', { pathData: { value: '/test/',  strict: false }});
    expect(match && match.url) .toBe('/test');

    match = matchPath('/test', { pathData: { value: '/test/',  strict: true }});
    expect(match).toBeFalsy();
  ...
  test('matchPath - hashType', () => {
    let match = matchPath('/test/123', { pathData: { value: '/test'}, hashType: true });
    expect(match).toBeFalsy();

    match = matchPath('/test/#home', { pathData: { value: '/home'}, hashType: true });
  ...
  test('checkActive', () => {
    ...
    checkFnc = checkActive({ exact: true }, '/test');
    ...
    checkFnc = checkActive({ exact: true }, ['/test', '/seller']);
    expect(checkFnc('/test/123')).toBeFalsy();
